...
function approve_deployment() {
  ...
  echo "Starting $run_id $runner" >> run.txt
  ...
  while true; do
    ...
    if [[ $status == "completed" ]]; then
      ...
      echo "=====> Run ID $run_id $runner completed" >> run.txt
      return
    elif [[ $status == "waiting" ]]; then
      # Get actions that need to be approved
      echo "Approving for $runner $run_id" >> run.txt
      ...
      echo "$environments $run_id $runner completed" >> run.txt
      ...
      echo "Waiting for $run_id $runner" >> run.txt
      sleep 120
      continue
    fi
  done
...
file_and_sha="ockam.aarch64-apple-darwin:f72eefc172d7ac7a1409965d9fa46ae519d5321bff4196eb52a8d996c07edf65 ockam.aarch64-unknown-linux-gnu:9ad054054c78ea8f45db43839317d769440483284551efa5dd04b8c304188e42 ockam.x86_64-apple-darwin:422b324b588d520fa9213e31b4f12ebaadf5ef9ad4674d5cd10635f6659d8bdf ockam.x86_64-unknown-linux-gnu:b697e5408d2dfe578b73f8854f8f372862939afe7e060d4d14890a9d6aae4ffa"

# temp_dir=$(mktemp -d)
# pushd $temp_dir
# gh release download $latest_tag_name -R $owner/ockam

# # TODO Ensure that SHA are same and that they are valid
